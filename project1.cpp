...
int birthday() {
    ...
    while (1) {
        ...
        srand(time(0));
        int i = rand();
        string t1 = rand_str(i);
        strcpy(mm1, t1.c_str());

        srand(time(0));
        int j = rand();
        string t2 = rand_str(j);
        strcpy(mm2, t2.c_str());
        sm3((char*)mm1, hash_temp1);
        sm3((char*)mm2, hash_temp2);
        if (!memcmp(hash_temp1, hash_temp2, 2)) {
            dump_buf((char*)hash_temp, 32);//碰撞到的信息的Hash
            break;
        }
    }
    return 0;
